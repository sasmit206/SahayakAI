  ...
  if (scheme.level.toLowerCase() === 'state' && scheme.states.length > 0) {
    if (profile.state) {
      ...
      if (stateMatch) {
        ...
        isEligible = false;
        reasons.push(
          `Ineligible: Scheme is only for residents of ${scheme.states.join(', ')} but applicant resides in ${profile.state}`
        );
      }
    } else {
  ...
  if (scheme.gender !== 'All') {
    if (profile.gender) {
      if (profile.gender.toLowerCase() === scheme.gender.toLowerCase()) {
        ...
        isEligible = false;
        reasons.push(
          `Ineligible: Scheme is restricted to ${scheme.gender} but applicant is ${profile.gender}`
        );
      }
    } else {
  ...
  if (scheme.incomeLimit !== null && scheme.incomeLimit > 0) {
    if (profile.income !== null) {
      if (profile.income <= scheme.incomeLimit) {
        ...
        reasons.push(
          `Income eligible: Family income ₹${profile.income.toLocaleString('en-IN')} is below the limit of ₹${scheme.incomeLimit.toLocaleString('en-IN')}`
        );
      } else {
        isEligible = false;
        reasons.push(
          `Ineligible: Family income ₹${profile.income.toLocaleString('en-IN')} exceeds the limit of ₹${scheme.incomeLimit.toLocaleString('en-IN')}`
        );
      }
    } else {
      reasons.push(
        `Income check pending (scheme income limit: ₹${scheme.incomeLimit.toLocaleString('en-IN')})`
      );
    }
  } else {
    score += SCORING_WEIGHTS.INCOME_MATCH;
  ...
  if (scheme.minAge !== null || scheme.maxAge !== null) {
    if (profile.age !== null) {
      ...
      if (scheme.minAge !== null && profile.age < scheme.minAge) {
        ageEligible = false;
        reasons.push(
          `Ineligible: Applicant age (${profile.age}) is below the minimum age of ${scheme.minAge}`
        );
      }
      if (scheme.maxAge !== null && profile.age > scheme.maxAge) {
        ageEligible = false;
        reasons.push(
          `Ineligible: Applicant age (${profile.age}) is above the maximum age of ${scheme.maxAge}`
        );
      }
  ...
  if (scheme.occupations.length > 0 && !scheme.occupations.includes('Other')) {
    if (profile.occupation) {
      ...
      if (occMatch) {
        ...
        isEligible = false;
        reasons.push(
          `Ineligible: Scheme targets ${scheme.occupations.join(', ')} but applicant is a ${profile.occupation}`
        );
      }
    } else {
  ...
  if (scheme.categories.length > 0) {
    if (profile.category) {
      ...
      if (catMatch) {
        ...
        isEligible = false;
        reasons.push(
          `Ineligible: Scheme is for ${scheme.categories.join(', ')} but applicant category is ${profile.category}`
        );
      }
    } else {
