...
export interface SchemeFormConfig {
  schemeId: string;
  schemeName: string;
  fields: FormField[];
}

export const SCHEME_FORM_CONFIGS: Record<string, SchemeFormConfig> = {
...
export const DEFAULT_FORM_CONFIG = (schemeId: string, schemeName: string): SchemeFormConfig => ({
  schemeId,
  schemeName,
  fields: [
...
export function getFormConfigForScheme(schemeId: string, schemeName: string): SchemeFormConfig {
  // Try matching by key
  if (SCHEME_FORM_CONFIGS[schemeId]) {
    return SCHEME_FORM_CONFIGS[schemeId];
  }
  
  // Or check if slug contains words in config keys
  for (const key of Object.keys(SCHEME_FORM_CONFIGS)) {
    if (schemeId.includes(key) || key.includes(schemeId)) {
      return SCHEME_FORM_CONFIGS[key];
    }
  }

  // Fallback to default
  return DEFAULT_FORM_CONFIG(schemeId, schemeName);
}
