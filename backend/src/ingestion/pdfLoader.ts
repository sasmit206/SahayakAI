...
export interface PdfExtractionResult {
  text: string;
  method: 'pdf-parse' | 'ocr' | 'pdf-parse+ocr';
  pageCount: number;
}
...
export async function extractTextFromPdf(filePath: string): Promise<PdfExtractionResult> {
  ...
  console.log(`[pdfLoader] "${path.basename(filePath)}" looks like a scanned PDF (only ${nativeText.length} chars extracted). Running OCR fallback...`);
...
async function ocrPdf(filePath: string): Promise<string> {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sahayak-ocr-'));
