import { initializeDbService, getAllSchemes } from '../services/dbService';
import { getRecommendations } from '../recommendation/recommendationEngine';
import { CitizenProfile } from '../profile/profileExtractor';
import { evaluateEligibility } from '../recommendation/eligibilityEngine';

interface EvaluationTestCase {
  profile: CitizenProfile;
  query: string;
  expectedSchemeId: string;
}
...
async function runEvaluation() {
  ...
  await initializeDbService(ingestLimit);
  ...
  for (let i = 0; i < TEST_CASES.length; i++) {
    ...
    const { recommendations } = await getRecommendations(tc.profile, tc.query, 10);
    ...
    if (rank !== -1) {
      ...
      if (expectedScheme) {
        const eligibility = evaluateEligibility(tc.profile, expectedScheme);
