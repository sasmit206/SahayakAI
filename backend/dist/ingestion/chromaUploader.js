...
exports.uploadToChroma = uploadToChroma;
exports.addSchemesToChroma = addSchemesToChroma;
const chromadb_1 = require("chromadb");
const env_1 = require("../config/env");
const embeddingGenerator_1 = require("./embeddingGenerator");
async function uploadToChroma(schemes) {
    const client = new chromadb_1.ChromaClient({ path: env_1.config.CHROMA_URL });
    console.log(`Connecting to ChromaDB at ${env_1.config.CHROMA_URL}...`);
    ...
    for (let i = 0; i < schemes.length; i += batchSize) {
        ...
        const embeddings = await (0, embeddingGenerator_1.generateEmbeddingsBatch)(documents, 16);
 ...
 */
async function addSchemesToChroma(schemes) {
    if (schemes.length === 0)
        return;
    const client = new chromadb_1.ChromaClient({ path: env_1.config.CHROMA_URL });
    const collectionName = 'sahayak_schemes';
...
exports.default = uploadToChroma;
