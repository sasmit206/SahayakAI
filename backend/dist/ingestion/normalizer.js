...
function normalizeRow(row, index) {
    ...
    // Extract Gender
    let gender = 'All';
    const hasFemaleKeywords = fullTextForMatching.includes('women') ||
    ...
    const hasMaleKeywords = fullTextForMatching.includes('boys') ||
    ...
    const isExclusiveCategory = lowercaseName.includes('scheduled caste') ||
    ...
    if (fullTextForMatching.includes('obc') || fullTextForMatching.includes('other backward'))
        categories.push('OBC');
    if (fullTextForMatching.includes('sc') || fullTextForMatching.includes('scheduled caste') || fullTextForMatching.includes('scheduled castes'))
        categories.push('SC');
    if (fullTextForMatching.includes('st') || fullTextForMatching.includes('scheduled tribe') || fullTextForMatching.includes('scheduled tribes'))
        categories.push('ST');
    if (fullTextForMatching.includes('brahmin'))
        categories.push('Brahmin');
    if (fullTextForMatching.includes('general') || fullTextForMatching.includes('open category'))
        categories.push('General');
    if (!isExclusiveCategory) {
        if (!categories.includes('General'))
            categories.push('General');
        if (!categories.includes('OBC'))
            categories.push('OBC');
    }
    ...
    // Extract Occupations
    const occupations = [];
    if (fullTextForMatching.includes('farmer') || fullTextForMatching.includes('cultivator') || fullTextForMatching.includes('agriculture'))
        occupations.push('Farmer');
    if (fullTextForMatching.includes('fisherman') || fullTextForMatching.includes('fishermen') || fullTextForMatching.includes('fisherwoman') || fullTextForMatching.includes('fishery'))
        occupations.push('Fisherman');
    if (fullTextForMatching.includes('construction') || fullTextForMatching.includes('building worker') || fullTextForMatching.includes('unregistered laborer') || fullTextForMatching.includes('shramik'))
        occupations.push('Construction Worker');
    if (fullTextForMatching.includes('safai') || fullTextForMatching.includes('sanitation') || fullTextForMatching.includes('sweeper'))
        occupations.push('Sanitation Worker');
    if (fullTextForMatching.includes('student') || fullTextForMatching.includes('scholarship') || fullTextForMatching.includes('matric') || fullTextForMatching.includes('school'))
        occupations.push('Student');
    if (fullTextForMatching.includes('entrepreneur') || fullTextForMatching.includes('msme') || fullTextForMatching.includes('business') || fullTextForMatching.includes('powerloom') || fullTextForMatching.includes('self-employed'))
        occupations.push('Entrepreneur');
    if (fullTextForMatching.includes('teacher') || fullTextForMatching.includes('faculty') || fullTextForMatching.includes('lecturer') || fullTextForMatching.includes('professor') || fullTextForMatching.includes('educator') || fullTextForMatching.includes('teaching') || fullTextForMatching.includes('aicte') || fullTextForMatching.includes('induction/refresher'))
        occupations.push('Teacher');
    if (occupations.length === 0) {
        // Default to a broad set if not specified
        occupations.push('Other');
    }
    // Extract Disability
    const disabilityOnly = lowercaseName.includes('disability') ||
