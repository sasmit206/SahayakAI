...
exports.translateSchemeToHindi = translateSchemeToHindi;
/**
 * schemeTranslator
 * ------------------------------------------------------------------------
...
const CACHE_PATH = path_1.default.resolve(__dirname, '../../data/scheme_translations_hi.json');
let memoryCache = {};
...
function loadCache() {
    ...
    try {
        if (fs_1.default.existsSync(CACHE_PATH)) {
            memoryCache = JSON.parse(fs_1.default.readFileSync(CACHE_PATH, 'utf-8'));
        }
    }
    catch (err) {
...
function persistCache() {
    try {
        ...
        fs_1.default.writeFile(CACHE_PATH, JSON.stringify(memoryCache, null, 2), (err) => {
            if (err)
...
function englishFallback(scheme) {
    return {
...
async function translateSchemeToHindi(scheme) {
        ...
        try {
            ...
            const response = await groq.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You translate Indian government welfare scheme descriptions from English to Hindi (Devanagari script) ' +
...
exports.default = translateSchemeToHindi;
