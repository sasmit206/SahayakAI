/**
 * App.tsx
 * Root component. Every API call now includes `language` from the
 * LanguageContext so the backend LLM responds in the correct language.
 */
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import {
  sahayakApi, CitizenProfile, ChatMessage,
  SchemeRecommendation, FormField,
} from './services/api';
...
export const App: React.FC = () => {
  const { t, language } = useLang();   // language sent with every API call
  ...
  const handleSendMessage = async (msg: string) => {
    ...
    try {
      // language is passed here — backend LLM will reply in selected language
      ...
      setMessages((prev) => [...prev, {
        ...
        content: '26a0Fe0f Connection error. Please verify the backend is running.',
        timestamp: Date.now(),
      }]);
    } finally {
      setIsLoading(false);
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, {
        ...
        content: '⚠️ Failed to reset the session.',
        timestamp: Date.now(),
      }]);
    } finally {
      setIsLoading(false);
  ...
  const handleApplyScheme = async (schemeId: string, schemeName: string) => {
    setIsLoading(true);
    try {
      const data = await sahayakApi.selectScheme(sessionId, schemeId, schemeName, language);
      setActiveMode(data.activeMode);
      ...
      if (error.response?.status === 404) {
        alert('Session expired. Please reload to start a new session.');
      } else {
        alert('Failed to start application flow.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAnswerSubmit = async (val: any) => {
    setIsLoading(true);
    try {
      const data = await sahayakApi.submitAnswer(sessionId, val, language);
      setActiveMode(data.activeMode);
      setNextQuestion(data.nextQuestion);
      setApplicationAnswers(data.applicationAnswers || {});
      setMessages(data.messages);
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen overflow-hidden bg-ink-950 text-ink-100">
      <header className="flex-shrink-0 border-b border-white/[0.06] bg-ink-950/90 backdrop-blur-md px-6 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            <span className="grid place-items-center h-6 w-6 rounded-md bg-white text-ink-950 font-display font-bold text-[11px]">S</span>
            <div className="flex items-baseline gap-2">
              <span className="font-display font-semibold tracking-tight text-[14px] text-white">{t.appName}</span>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex items-center gap-2">
            <span className="pill-neutral"><Database className="h-3 w-3" />{t.versionLabel}</span>
            <span className="pill-success"><ShieldCheck className="h-3 w-3" />{t.engineLabel}</span>
          </div>
          <LanguageSwitcher />
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden p-5 gap-5">
        <div className="w-[28%] min-w-[300px] h-full flex-shrink-0">
